...
function prompt_command() {
  ...
  PS1="
%B%F{31}${icon_start}%{$reset_color%}%F{black}%K{31}  %T %F{}%K{}$(virtualenv_prompt)${icon_user}%B%F{31}%n%{$reset_color%} ${icon_point} %F{251}${icon_directory}%~%{$reset_color%}\$([[ -n \$(git branch 2> /dev/null) ]] && echo \" ${icon_point}%F{172} ${icon_branch} \$(git_prompt_info) \$(git_prompt_short_sha)%{$reset_color%}\")
